...
export async function POST(req: NextRequest) {
  ...
  // Set successUrl to purchase confirmation page with placeholder for orderId
  // Set successUrl to generic confirmation page (no orderId)
  const successUrl = body.successUrl || `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/purchase-confirmation`;

    const subtotal = items.reduce((sum, it) => sum + (Number(it.price) || 0) * Math.max(1, Math.floor(it.qty || 1)), 0);
